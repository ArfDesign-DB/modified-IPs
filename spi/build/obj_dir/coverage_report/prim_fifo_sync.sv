...
%000000	  input                   clr_i,
	  // write port
 000974	  input                   wvalid_i,
 000013	  output                  wready_o,
 002205	  input   [Width-1:0]     wdata_i,
	  // read port
 000079	  output                  rvalid_o,
 000560	  input                   rready_i,
 001114	  output  [Width-1:0]     rdata_o,
	  // occupancy
%000008	  output                  full_o,
 001311	  output  [DepthW-1:0]    depth_o,
%000000	  output                  err_o
	);
 ...
 000031	        under_rst <= 1'b1;
%000004	      end else if (under_rst) begin
 013230	      verilator_coverage: (next point on previous line)

%000002	        under_rst <= ~under_rst;
	      end
	    end
 ...
 026524	      always_ff @(posedge clk_i)
 000882	        if (fifo_incr_wptr) begin
 012821	        verilator_coverage: (next point on previous line)

 000441	          storage[fifo_wptr] <= wdata_i;
	        end
	    end
