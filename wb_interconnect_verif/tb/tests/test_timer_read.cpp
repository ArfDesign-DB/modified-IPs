                    ...
                    VerilatedVcdC* trace)
{
    ...
    std::cout << "timer_we_o    : " << (int)dut->timer_we_o << "\n";

    std::cout << std::hex;
    std::cout << "timer_addr_o  : 0x" << dut->timer_addr_o << "\n";
    std::cout << std::dec;

    std::cout << "timer_be_o    : " << (int)dut->timer_be_o << "\n";

    if (dut->timer_req_o)
        std::cout << "[TIMER SELECTED]\n";

    dut->timer_rdata_i  = 0xCAFEBABE;
