...
//--------------------------------------------------
// Simple I2C Slave BFM
//--------------------------------------------------

void i2c_slave_bfm(Vi2c_master_top* top)
{
    //--------------------------------------------------
    // Default bus idle state
    //--------------------------------------------------

    top->sda_pad_i = 1;

    //--------------------------------------------------
    // Generate ACK from slave
    //--------------------------------------------------
    // When master releases SDA during ACK cycle,
    // slave pulls SDA low
    //--------------------------------------------------

    if(top->sda_padoen_o == 1 &&
       top->scl_pad_o    == 1)
    {
        top->sda_pad_i = 0;

        std::cout << "[SLAVE] ACK Generated\n";
    }
}
